...
const ModalCreateCard: FC<Modal> = ({ modal }) => {
  ...
  return (
    ...
    <Row md={12} className='modal-page'>
      <Col className='modal-page-container d-flex flex-column justify-content-center align-items-center'>
              ...
              <Col><div className='modal-subtitle'>Вам необходимо перезвонить специалисту для подтверждения </div></Col>

              <Row className='d-flex flex-column'>
                <Col className='mb-2'><MyButtonModal title='Елизавета' img={modalPhone} imgAlt='modal-phone' onClick={() => {sendMessageToTg('elizabethspoon')}}></MyButtonModal></Col>
                <Col className='mb-4'><MyButtonModal title='Екатерина' img={modalPhone} imgAlt='modal-phone' onClick={() => {sendMessageToTg('katerinaboynova')}}></MyButtonModal></Col>
              </Row>
